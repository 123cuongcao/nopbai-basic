...
export default function Index() {
  ...
  const [word, setWord] = useState(0); // Initialize with 0
  const [letter, setLetter] = useState(0); // Initialize with 0
  const [paragraph, setParagraph] = useState(0); // Initialize with 0

  const countLetter = (text) => {
    const textSplit = text.split(" ").join(""); // Remove spaces
    ...
    <div style={{ background: "linear-gradient(to right,#e002ec86,#f3b6b9db)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: "20px 0px 10px 0px" }}>
        <button
          ...
          onClick={handleUppercase} // Removed unnecessary arrow function
        >
          Click to UpperCase
        </button>
        <button
          ...
          onClick={handleLowerCase} // Removed unnecessary arrow function
        >
          Click to LowerCase
        </button>
      </div>
